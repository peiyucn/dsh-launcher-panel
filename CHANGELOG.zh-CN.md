...
- Dashboard 上的运行模式切换（pkg / src）从两个圆形图标钮改成普通药丸按钮，选中项以主色实心显示，与界面里其它按钮一致。
- dsh 版本号的写法在面板各处统一为 `v<版本号>`：源码模式下运行版本行不再带 `dsh-` 前缀（如显示 `v0.1.2-rc.1` 或 `v0.1.2-rc.1-99-g76fda72`，与 pkg 模式一致），Update 按钮与活动日志同样写作 `v<版本号>`。

## [0.2.11]

- 无用户可感知的变化：本版只更新了构建期依赖，扩展的行为与 0.2.10 完全一致。

## [0.2.10]

- Dashboard 的加载动画改为 dsh 官方的点阵样式：启动/安装/停止中的状态点、控制台里进行中的标记、以及打开面板时的加载状态。
- Check updates / Update 现在能识别同版本 rc 之后发布的正式版（如 0.1.5-rc.1 之后的 0.1.5），不再误报"已是最新"。
- 服务器在打开端口前退出且没有任何输出时，控制台会说明所配 Node 无法运行 dsh 的 CLI（Node 24.0/24.1 —— dsh 需要 Node 22.18+ 或 24.2+）以及该怎么办。
- 源码模式在 macOS/Linux 上需要 C 工具链（当前 dsh 的构建会编译原生组件）；README 环境章节已注明。
- README 不再声明 dsh 的极简模式在 Windows 下不可用——当前 dsh 已支持。
...
- Start 不再被后台重装卡住——那次重装可能因注册表问题失败，把本可运行的已装 dsh 挡在门外。
...
- Browser 下拉支持键盘导航（方向键 / Home / End，Enter 选择）。
- 错误提示统一遮蔽用户路径（安装/克隆进度行保留全路径，便于确认落点）。

## [0.2.4]

- Dashboard 视觉风格适配 dsh 官方设计。
